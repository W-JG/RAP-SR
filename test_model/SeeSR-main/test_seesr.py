...
def load_state_dict_diffbirSwinIR(model: nn.Module, state_dict: Mapping[str, Any], strict: bool = False) -> None:
    ...
    is_model_key_starts_with_module = list(model.state_dict().keys())[
        0].startswith("module.")
    is_state_dict_key_starts_with_module = list(
        state_dict.keys())[0].startswith("module.")

    if (
        is_model_key_starts_with_module and
        (not is_state_dict_key_starts_with_module)
    ):
        state_dict = {f"module.{key}": value for key,
                      value in state_dict.items()}
    if (
        (not is_model_key_starts_with_module) and
        is_state_dict_key_starts_with_module
    ):
        state_dict = {key[len("module."):]: value for key,
                      value in state_dict.items()}
...
def load_seesr_pipeline(args, accelerator, enable_xformers_memory_efficient_attention):
    ...
    feature_extractor = CLIPImageProcessor.from_pretrained(
        f"{args.pretrained_model_path}/feature_extractor")
    # unet = UNet2DConditionModel.from_pretrained(
    #     args.seesr_model_path, subfolder="unet")
    unet = UNet2DConditionModel.from_pretrained_orig(
        pretrained_model_path = args.unet_model_path,
        seesr_model_path = args.seesr_model_path,
        subfolder="unet",
    ...
    if accelerator.is_main_process:
        ...
        for image_idx, image_name in enumerate(image_names[:]):
            ...
            validation_prompt, ram_encoder_hidden_states = get_validation_prompt(
                args, validation_image, model)
            # clean, extremely detailed, best quality, sharp, clean
            validation_prompt = args.added_prompt + validation_prompt
            # dirty, messy, low quality, frames, deformed,
            ...
            for sample_idx in range(args.sample_times):
                os.makedirs(
                    f'{args.output_dir}/sample{str(sample_idx).zfill(2)}/', exist_ok=True)
...
if __name__ == "__main__":
    ...
    # user can add self-prompt to improve the results
    ...
    parser.add_argument("--mixed_precision", type=str,
                        default="fp16")  # no/fp16/bf16
    parser.add_argument("--guidance_scale", type=float, default=5.5)
    parser.add_argument("--conditioning_scale", type=float, default=1.0)
    parser.add_argument("--blending_alpha", type=float, default=1.0)
    parser.add_argument("--num_inference_steps", type=int, default=50)
    parser.add_argument("--process_size", type=int, default=512)
    parser.add_argument("--vae_decoder_tiled_size", type=int,
                        default=224)  # latent size, for 24G
    parser.add_argument("--vae_encoder_tiled_size", type=int,
                        default=1024)  # image size, for 13G
    ...
    # LR Embedding Strategy, choose 'lr latent + 999 steps noise' as diffusion start point.
