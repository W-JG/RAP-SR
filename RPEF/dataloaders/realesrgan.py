...
from dataloaders.utils.image import DiffJPEG, USMSharp, filter2D, img2tensor
...
class RealESRGAN_degradation(object):
    ...
    def degrade_process(self, img_gt, resize_bak=False):
        ...
        img_gt, kernel1, kernel2, sinc_kernel = (
            img_gt.to(self.device),
            kernel1.to(self.device),
            kernel2.to(self.device),
            sinc_kernel.to(self.device),
        )
        ...
        out = F.interpolate(
            out,
            size=(int(ori_h / scale_final * scale), int(ori_w / scale_final * scale)),
            mode=mode,
        )
        # noise
        ...
        if np.random.uniform() < 0.5:
            ...
            mode = random.choice(["area", "bilinear", "bicubic"])
            out = F.interpolate(
                out, size=(ori_h // scale_final, ori_w // scale_final), mode=mode
            )
        ...
        # clamp and round
        img_lq = torch.clamp((out * 255.0).round(), 0, 255) / 255.0
